...
namespace Gungeon
{
	...
	public:
		...
		void HitBullet() const;
		void SpawnByForceInMiro(const Vector2 wpos);
		void SpawnAnim() const;
		void SpawnAnimEnd() const;
		void CutSceneOn() const;
		void CutSceneOff() const;
	...
	private:
		...
		float						intervalFire[static_cast<int>(BossPattern::max)];
		float						intervalEnd[static_cast<int>(BossPattern::max)];
